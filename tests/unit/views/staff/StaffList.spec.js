...
describe('StaffList view', () => {
  ...
  it('検索結果が0件の場合、検索結果一覧が表示されないこと', async () => {
    ...
    const noResult = wrapper.find('#no-result');
    expect(noResult.isEmpty()).toBe(false);
  });
  ...
  it('検索結果が存在する場合、検索結果の2件目が検索結果の2件目と一致すること', async () => {
    ...
    expect(cols.at(1).text()).toBe('2'); //id
    expect(cols.at(2).text()).toBe('robert'); //firstname
    expect(cols.at(3).text()).toBe('de niro'); //lastname
    expect(cols.at(4).text()).toBe('sample@sample.com'); //email
    expect(cols.at(5).text()).toBe('09012345678'); //tel
  });
});
